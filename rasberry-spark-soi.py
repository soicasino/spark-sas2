...
class ConfigManager:
    def __init__(self, config_file_path="settings.ini"):
        ...
        # Does not read on init anymore. Starts fresh.
        # Defaults are applied after port detection or when explicitly called.

    def _create_default_config_sections(self):
        """Creates default sections if they don't exist. Called before setting specific defaults."""
    ...
    def apply_operational_defaults(self):
        """
        Applies default operational (non-port) settings.
        This should be called after port detection and before saving the initial config.
        """
        self._create_default_config_sections()

        # SAS Defaults (excluding port)
        self.set_value('sas', 'address', self.get('sas', 'address', '01'))
        self.set_value('sas', 'assetnumber', self.get('sas', 'assetnumber', '00000000')) # This might be updated by AssetManager
        ...
        # CardReader Defaults (excluding port_or_path, but keeping type and lib_path as they guide detection)
        ...
        # BillAcceptor Defaults (excluding port, but keeping typeid)
    ...
    def getdecimal(self, section, option, fallback=Decimal('0.0')):
        if self.config.has_option(section, option):
            try:
                return Decimal(self.config.get(section, option))
            except Exception:
                return fallback
        return fallback
    ...
    def set_value(self, section, option, value):
        ...
        elif self.config.has_option(section, option): # If value is None, remove existing option
    ...
    def save(self):
        try:
            ...
            self.logger(f"Error saving config file to {self.config_file_path}: {e}")

# Removed global app_config instance
...
class PortManager:
    def __init__(self, config_manager_instance): # Takes ConfigManager instance
        self.config_manager = config_manager_instance # Use the passed instance
        self.available_ports = []
        self.sas_port_name = None
        self.card_reader_port_name = None # This will store the detected serial port or lib path
        self.bill_acceptor_port_name = None
        self.logger = self.config_manager.logger # Use logger from config_manager
    ...
    def find_available_serial_ports(self):
        ...
        if platform.system().startswith("Window"):
            ...
            except ImportError: self.logger("pyserial list_ports not available. Manual config needed for Windows.")
    ...
    def _assign_port_locally(self, port_name, device_name):
        """Internal helper to mark port as used in self.available_ports."""
        for port_info in self.available_ports:
            if port_info["port_no"] == port_name:
                port_info["is_used"] = True
                port_info["device_name"] = device_name
                self.logger(f"Locally assigned port {port_name} to {device_name} for testing.")
                return True
        return False

    def find_and_assign_sas_port(self):
        self.logger("Attempting to find SAS port...")
        for port_info in [p for p in self.available_ports if not p["is_used"]]: # Iterate unassigned
            port_name = port_info['port_no']
            self.logger(f"Testing port {port_name} for SAS...")
            temp_sas_comm = SASCommunicator(port_name, self.config_manager) # Pass ConfigManager instance
            if temp_sas_comm.open_port():
                temp_sas_comm.get_sas_version_and_serial()
                time.sleep(0.2)
                response = temp_sas_comm.receive_data()
                temp_sas_comm.close_port()
                if response and response.startswith(temp_sas_comm.sas_address + "54"):
                    self.logger(f"SAS device identified on {port_name}")
                    self._assign_port_locally(port_name, "SAS")
                    self.sas_port_name = port_name
                    return port_name
                elif response : # Any SAS response is a good sign
                    self.logger(f"Potential SAS device on {port_name} (general response: {response}). Assigning.")
                    self._assign_port_locally(port_name, "SAS")
                    self.sas_port_name = port_name
                    return port_name
    ...
    def find_and_assign_card_reader_port(self):
        ...
        if reader_type_id == 1: # CRT288B
            self.card_reader_port_name = self.config_manager.get('cardreader', 'lib_path', './crt_288B_UR.so')
            self.logger(f"CRT288B card reader type selected; using configured path: {self.card_reader_port_name}")
            # No serial port to "assign" from available_ports for lib-based reader
        ...
        for port_info in [p for p in self.available_ports if not p["is_used"]]:
            ...
            if temp_card_reader._open_serial_port():
                ...
                if reader_type_id == 2: # rCloud
                    temp_card_reader._send_rcloud_command("02000235310307") # Get UID
                    time.sleep(0.1)
                    response = temp_card_reader._receive_rcloud_data()
                    if response and (response.startswith("020003") or response.startswith("020007") or response == "06"):
                        ...
                        self._assign_port_locally(port_name, "CardReader")
                        self.card_reader_port_name = port_name
                        temp_card_reader._close_serial_port()
                        return port_name
                temp_card_reader._close_serial_port()
                if response: self.logger(f"No conclusive Card Reader response from {port_name}, got: {response}")
            else: self.logger(f"Could not open {port_name} for Card Reader test.")
        self.logger("Serial Card Reader port not found.")
        return None

    def find_and_assign_bill_acceptor_port(self):
        acceptor_type_id = self.config_manager.getint('billacceptor', 'typeid', 0)
        if acceptor_type_id == 0:
            self.logger("Bill Acceptor is SAS controlled, no separate port to find.")
            return None
        self.logger("Attempting to find direct serial Bill Acceptor port...")
        for port_info in [p for p in self.available_ports if not p["is_used"]]:
            port_name = port_info['port_no']
            self.logger(f"Testing port {port_name} for Bill Acceptor (type {acceptor_type_id})...")
            temp_ba_handler = BillAcceptorHandler(port_name, self.config_manager, acceptor_type_id)
            if temp_ba_handler._open_direct_port():
                temp_ba_handler.request_status()
                time.sleep(0.2)
                response = temp_ba_handler._receive_direct_data()
                temp_ba_handler._close_direct_port()
                if response:
                    self.logger(f"Bill Acceptor (type {acceptor_type_id}) identified on {port_name}")
                    self._assign_port_locally(port_name, "BillAcceptor")
                    self.bill_acceptor_port_name = port_name
                    return port_name
            else: self.logger(f"Could not open {port_name} for Bill Acceptor test.")
        self.logger("Direct serial Bill Acceptor port not found.")
        return None


class SASCommunicator:
    def __init__(self, port_name, config_manager_instance, baud_rate=19200, timeout=0.1): # Takes ConfigManager instance
        ...
        self.transaction_id = int(self.config_manager.get('payment', 'transactionid', 0))
        ...
        if self.use_termios_for_parity:
            try:
                ...
                self.CMSPAR = getattr(termios, 'CMSPAR', 0o10000000000)
                if self.CMSPAR == 0o10000000000 and not hasattr(termios, 'CMSPAR'):
                     self.logger("termios.CMSPAR not found, using fallback value for CMSPAR.")
            except ImportError: self.logger("Warning: termios module not found."); self.use_termios_for_parity = False
            except AttributeError: self.logger("Warning: CMSPAR not found."); self.use_termios_for_parity = False
    
    def open_port(self):
        if self.is_port_open: self.logger(f"SAS port {self.port_name} is already open."); return True
        try:
            ...
            self.logger(f"SAS port {self.port_name} opened successfully.")
            device_type_id = self.config_manager.getint('machine', 'devicetypeid', 8)
            if device_type_id in [1, 4]: # Novomatic/Octavian
                self.logger("Device type Novomatic/Octavian, setting EVEN parity after initial polls.")
                self._send_raw_command_platform_specific(f"{self.sas_address}80"); time.sleep(0.05)
                self._send_raw_command_platform_specific(f"{self.sas_address}81"); self.serial_port.close()
                self.serial_port.parity = serial.PARITY_EVEN; self.serial_port.open()
                self.logger(f"SAS port {self.port_name} re-opened with EVEN parity.")
            else: self._send_raw_command_platform_specific(f"{self.sas_address}80"); time.sleep(0.05)
            return True
    ...
    def _send_raw_command_platform_specific(self, command_hex_full):
        ...
        device_type_id = self.config_manager.getint('machine', 'devicetypeid', 8)
        if device_type_id in [1, 4] and self.serial_port.parity != serial.PARITY_EVEN:
             self.logger(f"Warning: Parity for Novomatic/Octavian should be EVEN, but is {self.serial_port.parity}.")
        if self.use_termios_for_parity and len(data_bytes) > 0:
            try:
            ...
            except Exception as e: self.logger(f"Error setting termios parity: {e}. Sending with default."); self.serial_port.write(data_bytes)
        else: self.serial_port.write(data_bytes)

    def send_general_poll(self):
        if not self.is_port_open: return
        poll_byte_hex = "81" if self.last_sent_poll_type == 80 else "80"
        full_poll_command = self.sas_address + poll_byte_hex
        self._send_raw_command_platform_specific(full_poll_command)
        self.last_sent_poll_type = int(poll_byte_hex, 16)

    def send_command(self, command_name, command_body_hex, add_address_and_crc=True):
        if not self.is_port_open: self.logger(f"Cannot send '{command_name}', port not open."); return
        full_command_hex = self.sas_address + command_body_hex if add_address_and_crc else command_body_hex
        if add_address_and_crc: full_command_hex = get_crc(full_command_hex)
        self.logger(f"TX SAS ('{command_name}'): {full_command_hex}")
        self._send_raw_command_platform_specific(full_command_hex)
    ...
    def _parse_sas_message(self, message_hex):
        ...
        if received_address == self.sas_address:
            ...
            elif command_byte == "FF": parsed_data["type"] = "GENERAL_EXCEPTION"; parsed_data["exception_code"] = message_hex[4:6] if len(message_hex) >=6 else None
        elif len(message_hex) == 2 and (message_hex == "80" or message_hex == "81"): parsed_data["type"] = "POLL_ECHO_OR_COLLISION"
        return parsed_data
    ...
    def process_received_message(self, message_hex):
        ...
        if parsed_message: self.logger(f"Parsed SAS Message: {parsed_message}")
...
class AssetManager:
    def __init__(self, sas_communicator, config_manager_instance): # Takes ConfigManager instance
        self.sas_comm = sas_communicator
        self.config_manager = config_manager_instance # Use the passed instance
    ...
    def _get_asset_binary(self, asset_int):
        ...
        return reversed_hex_str.ljust(8, '0')[:8]

    def _read_asset_to_int(self, reversed_hex_str_8_chars):
        if len(reversed_hex_str_8_chars) != 8: self.logger(f"Warning: Asset hex '{reversed_hex_str_8_chars}' not 8 chars."); return 0
        normal_order_hex = "".join(reversed([reversed_hex_str_8_chars[i:i+2] for i in range(0, len(reversed_hex_str_8_chars), 2)]))
        try: return int(normal_order_hex, 16)
        except ValueError: self.logger(f"Error converting asset hex '{normal_order_hex}' to int."); return 0
    ...
    def update_asset_info_from_sas_response(self, parsed_sas_message):
        ...
        if asset_hex_from_resp and len(asset_hex_from_resp) == 8:
            ...
            if new_asset_int != 0 and new_asset_int != self.current_asset_number_int :
                ...
                self.current_asset_number_int = new_asset_int
                self.current_asset_number_hex_reversed = asset_hex_from_resp
                self.config_manager.set_value('sas', 'assetnumber', self.current_asset_number_hex_reversed)
            elif new_asset_int == 0 and asset_hex_from_resp != "00000000": self.logger(f"Warning: Read asset {asset_hex_from_resp} converted to 0.")

    def read_asset_from_machine(self): self.logger("Requesting asset number from EGM..."); self.sas_comm.get_asset_number_info()

    def register_asset_on_machine(self, asset_number_to_register_int=None, mode="register"):
        asset_to_reg_int = asset_number_to_register_int if asset_number_to_register_int is not None else self.current_asset_number_int
        asset_hex_rev = self.current_asset_number_hex_reversed
        if asset_to_reg_int != 0: asset_hex_rev = self._get_asset_binary(asset_to_reg_int)
        elif self.current_asset_number_int == 0 and self.current_asset_number_hex_reversed == '00000000':
             self.logger("Asset num is 0 and not configured. Cannot effectively register asset 0.")
        reg_mode = "00" if mode == "register" else "01"; pos_id = "00000000"
        cmd_body = f"731D{reg_mode}{asset_hex_rev}{self.registration_key}{pos_id}"
        self.logger(f"Registering asset: {asset_to_reg_int} (Hex: {asset_hex_rev}), Mode: {mode}")
        self.sas_comm.send_command("RegisterAssetNumber", cmd_body)

class AFTHandler: # Uses ConfigManager instance via self.config_manager
    ...
    def _get_next_transaction_id(self, increment=True):
        if increment:
            self.current_transaction_id = (self.current_transaction_id % 255) + 1
            self.config_manager.set_value('payment', 'transactionid', self.current_transaction_id)
    ...
    def _construct_aft_payload(self, transfer_type_byte, cash_amt, rest_amt, non_rest_amt, pool_id_hex):
        ...
        tx_id_hex = "".join(f"{ord(c):02x}" for c in str(tx_id))
        tx_id_len_hex = f"{int(len(tx_id_hex)/2):02x}".zfill(2)
        asset_num_hex = self.asset_manager.current_asset_number_hex_reversed
        reg_key_hex = self.asset_manager.registration_key
        payload = "0000" + transfer_type_byte # Transfer Code, Index, Type
        payload += self._format_amount_bcd(cash_amt, 5)
        payload += self._format_amount_bcd(rest_amt, 5)
        payload += self._format_amount_bcd(non_rest_amt, 5)
        transfer_flags = "07" # Default: Hard cashout
        if self.config_manager.getint('machine', 'config_is_cashout_soft', 0) == 1: transfer_flags = "03"
        # Adjust flags for handpay if transfer_type_byte implies it (e.g. "0B")
        if transfer_type_byte == "0B": transfer_flags = "0B" # Example for handpay request
        payload += transfer_flags + asset_num_hex + reg_key_hex + tx_id_len_hex + tx_id_hex
        payload += "00000000" + pool_id_hex + "00" # Expiration, PoolID, ReceiptLen
    ...
    def transfer_to_machine(self, cash_amt, rest_amt, non_rest_amt, transfer_type="00", pool_id="0000"):
        ...
        cmd_len_hex = f"{int(len(payload)/2):02x}".zfill(2)
        cmd_body = f"72{cmd_len_hex}{payload}"
        self.sas_comm.send_command("AFT_TransferToMachine", cmd_body)
        self.logger(f"AFT to machine: TxID {tx_id}, Cash {cash_amt}, R {rest_amt}, NR {non_rest_amt}")
        return True
    ...
    def transfer_from_machine(self, cash_amt, rest_amt, non_rest_amt, transfer_type="80", pool_id="0000"):
        ...
        # For cashout (type "80"), amounts in payload are often what EGM has, or can be 0s if EGM determines
        payload, tx_id = self._construct_aft_payload(transfer_type, cash_amt, rest_amt, non_rest_amt, pool_id)
        cmd_len_hex = f"{int(len(payload)/2):02x}".zfill(2)
        cmd_body = f"72{cmd_len_hex}{payload}"
        self.sas_comm.send_command("AFT_TransferFromMachine", cmd_body)
        self.logger(f"AFT from machine: TxID {tx_id}, ExpCash {cash_amt}, ExpR {rest_amt}")
        return True
        
    def update_transfer_status(self, sas_parsed_response):
        if sas_parsed_response and sas_parsed_response.get("type") == "AFT_RESPONSE":
            status_code = sas_parsed_response.get("transfer_status")
            self.last_transfer_status = status_code
...
class BillAcceptorHandler: # Uses ConfigManager instance via self.config_manager
    def __init__(self, port_name_or_sas_comm, config_manager_instance, acceptor_type_id):
        self.config_manager = config_manager_instance
        self.acceptor_type_id = acceptor_type_id
        self.port_name = None; self.serial_port = None; self.sas_communicator = None
        self.is_enabled_hw = False; self.is_enabled_sas = False
        self.mei_last_ack_nack = "10"; self.last_bill_escrowed = None
        self.logger = self.config_manager.logger
        if self.acceptor_type_id == 0:
            if not isinstance(port_name_or_sas_comm, SASCommunicator): raise ValueError("SASComm needed for SAS BA.")
            self.sas_communicator = port_name_or_sas_comm; self.logger("BA: SAS controlled.")
        else:
            self.port_name = port_name_or_sas_comm; self.baudrate = 9600
    ...
    def _get_mei_msg_crc(self, cmd_body_no_crc):
        temp_cmd = cmd_body_no_crc.replace(" ", "") + "00"; hex_data = decode_to_hex(temp_cmd)
        bcc = 0; 
    ...
    def poll_events(self): # Placeholder for detailed parsing
...
class CardReaderHandler: # Uses ConfigManager instance via self.config_manager
    def __init__(self, port_name_or_lib_path, config_manager_instance, reader_type_id):
        self.config_manager = config_manager_instance
        self.reader_type_id = reader_type_id
    ...
    def _open_serial_port(self):
        ...
        try:
            ...
            if self.reader_type_id == 2: # rCloud
                self.serial_port.bytesize = serial.EIGHTBITS; self.serial_port.parity = serial.PARITY_NONE
    ...
    def check_for_card(self): # Simplified event logic
        uid, present = None, False
        if self.reader_type_id == 1: # CRT288B
        ...
        elif self.reader_type_id == 2: # rCloud
...
class SlotMachineApplication:
    def __init__(self, config_file_path="settings.ini"):
        self.config_file_path = config_file_path
        self.config = None # Will be initialized in _setup_configuration_and_ports
        self.port_mgr = None # Will be initialized in _setup_configuration_and_ports
        ...
        self.running = False; self.sas_poll_timer = None; self.card_read_timer = None
        self.bill_acceptor_poll_timer = None
    ...
    def _delete_config_file(self):
        if os.path.exists(self.config_file_path):
            try:
                ...
                self.logger(f"Removed existing configuration file: {self.config_file_path}")
            except OSError as e:
                self.logger(f"Error removing configuration file {self.config_file_path}: {e}")

    def _setup_configuration_and_ports(self):
        self._delete_config_file() # Remove old config first
        self.config = ConfigManager(self.config_file_path) # Create a fresh ConfigManager
        self.logger = self.config.logger # Use logger from config manager
        self.port_mgr = PortManager(self.config) # Pass the new ConfigManager instance

        self.port_mgr.find_available_serial_ports()
        
        # Port detection sequence
        self.port_mgr.find_and_assign_sas_port()
        self.port_mgr.find_and_assign_card_reader_port() # Type is read from config by PortManager
        self.port_mgr.find_and_assign_bill_acceptor_port() # Type is read from config

        # Update ConfigManager with detected ports
        self.config.set_value('sas', 'port', self.port_mgr.sas_port_name)
        # Card reader port_or_path is handled slightly differently based on type
        # If serial, PortManager sets card_reader_port_name. If lib-based, it's the lib_path.
        ...
        # Apply other operational defaults (non-port related)
        self.config.apply_operational_defaults()
        
        self.config.save() # Save the newly created config with detected ports and defaults
        self.logger("Initial configuration and port detection complete.")

    def initialize_components(self):
        self.logger("Initializing components based on detected/configured ports...")
        # SAS Port and dependent components
        if self.port_mgr.sas_port_name:
            ...
            if self.sas_comm.open_port():
                self.asset_mgr = AssetManager(self.sas_comm, self.config)
                self.aft_handler = AFTHandler(self.sas_comm, self.config, self.asset_mgr)
                # self.asset_mgr.read_asset_from_machine() # Initial request
            else: self.logger(f"Failed to open SAS port {self.port_mgr.sas_port_name}"); self.sas_comm = None
        else: self.logger("SAS port was not assigned by PortManager.")

        # Card Reader
        if self.port_mgr.card_reader_port_name:
            reader_type = self.config.getint('cardreader', 'type', 2) # Type from (now saved) config
            self.card_reader_handler = CardReaderHandler(self.port_mgr.card_reader_port_name, self.config, reader_type)
            if reader_type != 1: # Serial based
                if not self.card_reader_handler._open_serial_port():
                    self.logger(f"Failed to open card reader port {self.port_mgr.card_reader_port_name}"); self.card_reader_handler = None
        else: self.logger("Card reader port/path was not assigned by PortManager.")

        # Bill Acceptor
        ba_type = self.config.getint('billacceptor', 'typeid', 0)
        if ba_type != 0: # Direct serial BA
            if self.port_mgr.bill_acceptor_port_name:
                ...
                if not self.bill_acceptor_handler._open_direct_port():
                    self.logger(f"Failed to open BA port {self.port_mgr.bill_acceptor_port_name}"); self.bill_acceptor_handler = None
            else: self.logger("Direct serial BA port was not assigned by PortManager.")
        elif self.sas_comm: # SAS controlled BA
             self.bill_acceptor_handler = BillAcceptorHandler(self.sas_comm, self.config, 0)
        else: self.logger("BA is SAS controlled, but SAS communicator is not available.")
        self.logger("Component initialization finished.")

    def _sas_polling_loop(self): # Same as before
        if not self.running or not self.sas_comm or not self.sas_comm.is_port_open: return
        self.sas_comm.send_general_poll()
        response_hex = self.sas_comm.receive_data(timeout_override=0.05)
        if response_hex:
        ...
        if self.running:
            self.sas_poll_timer = threading.Timer(0.04, self._sas_polling_loop); self.sas_poll_timer.daemon = True; self.sas_poll_timer.start()

    def _card_reader_loop(self): # Same as before
    ...
    def _bill_acceptor_loop(self): # Same as before
        if not self.running or not self.bill_acceptor_handler or self.bill_acceptor_handler.acceptor_type_id == 0: return
        self.bill_acceptor_handler.request_status()
        time.sleep(0.05)
        event_data = self.bill_acceptor_handler.poll_events()
        if event_data: self.logger(f"MainApp: Bill Acceptor Event - {event_data}")
        if self.running:
            self.bill_acceptor_poll_timer = threading.Timer(0.2, self._bill_acceptor_loop); self.bill_acceptor_poll_timer.daemon = True; self.bill_acceptor_poll_timer.start()

    def start(self):
        self._setup_configuration_and_ports() # New setup method
        self.initialize_components() # Initialize with detected/configured ports
        self.running = True
        # Start polling loops if components are available
        if self.sas_comm and self.sas_comm.is_port_open: self._sas_polling_loop()
        if self.card_reader_handler:
            if self.card_reader_handler.reader_type_id != 1 and (not self.card_reader_handler.serial_port or not self.card_reader_handler.serial_port.is_open):
                self.card_reader_handler._open_serial_port()
            if (self.card_reader_handler.reader_type_id == 1 and self.card_reader_handler.crt_lib) or \
               (self.card_reader_handler.reader_type_id != 1 and self.card_reader_handler.serial_port and self.card_reader_handler.serial_port.is_open):
                self._card_reader_loop()
        if self.bill_acceptor_handler and self.bill_acceptor_handler.acceptor_type_id != 0:
            if not self.bill_acceptor_handler.serial_port or not self.bill_acceptor_handler.serial_port.is_open:
                self.bill_acceptor_handler._open_direct_port()
            if self.bill_acceptor_handler.serial_port and self.bill_acceptor_handler.serial_port.is_open:
                self._bill_acceptor_loop()
        self.logger("Slot Machine Application Started. Press Ctrl+C to exit.")
        try:
            while self.running: time.sleep(1) 
        except KeyboardInterrupt: self.logger("Shutdown requested.")
        finally: self.shutdown()

    def shutdown(self): # Same as before
        self.logger("Shutting down application...")
        self.running = False
        ...
        if self.config: self.config.save() # Save any final config changes (e.g. last transaction ID)
        self.logger("Application shutdown complete.")

if __name__ == "__main__":
    app = SlotMachineApplication(config_file_path="settings.ini") # Pass the desired config file path
    app.start()
