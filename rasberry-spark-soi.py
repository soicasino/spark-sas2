...
import platform
import os
import fnmatch # Not used in this snippet, but kept from previous
import subprocess
import re # Not used in this snippet, but kept from previous
...
# Import termios only if on Linux, to prevent ImportError on Windows
if platform.system() == "Linux":
    import termios
else:
    termios = None # Placeholder for non-Linux systems
    ...
    def _assign_port_locally(self, port_name, device_name):
        for port_info in self.available_ports:
            if port_info["port_no"] == port_name:
                ...
                return True
        return False
    def find_and_assign_sas_port(self): # Logic remains similar to v3, relies on corrected SASCommunicator
...
class SASCommunicator:
    def __init__(self, port_name, config_manager_instance, baud_rate=19200, timeout=0.1):
        ...
        self.logger = self.config_manager.logger
        self.device_type_id = self.config_manager.getint('machine', 'devicetypeid', 8)
        
        self.is_platform_linux = platform.system() == "Linux"
        if self.is_platform_linux and termios is None:
            self.logger("WARNING: termios module could not be imported on Linux. Mark/Space parity will not work.")
            self.use_termios_for_parity = False
        elif self.is_platform_linux:
            self.use_termios_for_parity = True # Will be refined based on device_type_id in send method
            self.CMSPAR = 0x40000000 # Fixed value from original script
            self.logger(f"Linux platform: termios available. CMSPAR set to {hex(self.CMSPAR)}.")
        else:
            self.use_termios_for_parity = False
    
    def _open_port_base(self, for_test=False):
        """Base port opening logic."""
        ...
        try:
            ...
            log_suffix = "for test" if for_test else "for operation"
            self.logger(f"SAS port {self.port_name} opened {log_suffix} with PARITY_NONE.")
            
            if self.device_type_id in [1, 4]: 
                self.logger(f"Novomatic/Octavian (type {self.device_type_id}): initial polls with NONE, then switch to EVEN.")
                ...
                self.logger(f"SAS port {self.port_name} re-opened with PARITY_EVEN for Novomatic/Octavian {log_suffix}.")
            else: 
                self.logger(f"Sending initial '80' poll with PARITY_NONE for device type {self.device_type_id} {log_suffix}.")
                self.serial_port.write(decode_to_hex("80")); self.serial_port.flush()
                time.sleep(0.05) 
            return True
        except Exception as e: 
            log_suffix = "for test" if for_test else "for operation"
            self.logger(f"Error in open_sas_port_{log_suffix} ({self.port_name}): {e}")
            self.is_port_open = False
            return False

    def open_sas_port_for_test(self):
        return self._open_port_base(for_test=True)

    def open_port(self): 
        # Operational open might have slightly different logic if needed in future,
        # but for now, it's the same as the test open to ensure consistency.
        # The key is that after this, send_general_poll and send_command will use
        # _send_raw_command_platform_specific which applies termios correctly.
        return self._open_port_base(for_test=False)


    def close_port(self):
        if self.serial_port and self.serial_port.is_open:
            # Before closing, if termios was used, restore to a known safe state (e.g. PARITY_NONE)
            if self.is_platform_linux and termios and self.device_type_id not in [1, 4, 11]:
                try:
                    iflag, oflag, cflag, lflag, ispeed, ospeed, cc = termios.tcgetattr(self.serial_port.fileno())
                    cflag &= ~(termios.PARENB | self.CMSPAR | termios.PARODD) # Clear all parity bits for NONE
                    termios.tcsetattr(self.serial_port.fileno(), termios.TCSANOW, [iflag, oflag, cflag, lflag, ispeed, ospeed, cc])
                    self.logger(f"Restored port {self.port_name} to PARITY_NONE via termios before closing.")
                except Exception as e:
                    self.logger(f"Error restoring termios settings on close for {self.port_name}: {e}")
    ...
    def _send_raw_command_platform_specific(self, command_hex_full):
        ...
        apply_termios = self.use_termios_for_parity and \
                        self.is_platform_linux and \
                        termios is not None and \
                        self.device_type_id not in [1, 4, 11]

        if apply_termios:
            try:
                fd = self.serial_port.fileno()
                # Get current settings (port should be PARITY_NONE from open_port)
                iflag, oflag, cflag, lflag, ispeed, ospeed, cc = termios.tcgetattr(fd)
                original_cflag_for_restore = cflag # This cflag reflects PARITY_NONE settings
                
                # Set MARK parity for the first byte
                cflag_mark = (cflag | termios.PARENB | self.CMSPAR | termios.PARODD)
                cflag_mark &= ~termios.CSTOPB 
                termios.tcsetattr(fd, termios.TCSANOW, [iflag, oflag, cflag_mark, lflag, ispeed, ospeed, cc])
                self.serial_port.write(data_bytes[0:1]); self.serial_port.flush()
                
                if len(data_bytes) > 1: # Multi-byte command
                    time.sleep(0.001) # saswaittime from original script
                    # Set SPACE parity for subsequent bytes
                    # Important: get cflag again as it was changed to MARK
                    iflag_mark, oflag_mark, cflag_mark_current, lflag_mark, ispeed_mark, ospeed_mark, cc_mark = termios.tcgetattr(fd)
                    cflag_space = (cflag_mark_current | termios.PARENB | self.CMSPAR) & ~termios.PARODD # Start from MARK state and set SPACE
                    cflag_space &= ~termios.CSTOPB 
                    termios.tcsetattr(fd, termios.TCSANOW, [iflag_mark, oflag_mark, cflag_space, lflag_mark, ispeed_mark, ospeed_mark, cc_mark])
                    self.serial_port.write(data_bytes[1:]); self.serial_port.flush()
                    
                    # After a multi-byte command, restore to original_cflag_for_restore (PARITY_NONE settings)
                    termios.tcsetattr(fd, termios.TCSANOW, [iflag, oflag, original_cflag_for_restore, lflag, ispeed, ospeed, cc])
                # else: Single-byte command (e.g. poll). Port is left in MARK parity state by termios.
                # This matches the original script's behavior. The next send will re-evaluate.
            except Exception as e: 
                self.logger(f"Error setting termios Mark/Space parity: {e}.")
                # Attempt to restore original cflag on error if possible
                try: 
                    if 'original_cflag_for_restore' in locals() and 'fd' in locals():
                         iflag_err, oflag_err, _, lflag_err, ispeed_err, ospeed_err, cc_err = termios.tcgetattr(fd) # Get current iflags etc.
                         termios.tcsetattr(fd, termios.TCSANOW, [iflag_err, oflag_err, original_cflag_for_restore, lflag_err, ispeed_err, ospeed_err, cc_err])
                         self.logger("Attempted to restore original termios settings after error.")
                except: pass 
                self.logger(f"Falling back to direct write with port's current parity ({self.serial_port.parity}).")
                self.serial_port.write(data_bytes); self.serial_port.flush() # Fallback
        else: 
            # For Windows, or device types 1, 4, 11 (Novomatic, Octavian, Interblock), or if termios is disabled
    ...
    def send_general_poll(self): 
        ...
        # This will use Mark/Space if applicable for the device type & platform,
        # and leave the port in MARK state if it's a single byte termios send.
    ...
    def receive_data(self, timeout_override=None, expect_long_response=False): 
        ...
        try:
            ...
            initial_chunk = self.serial_port.read(max(1, self.serial_port.in_waiting or 0)) 
            if initial_chunk:
